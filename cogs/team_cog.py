...
def overwatch_role_need_score(team: list[dict], player: dict):
    ...
    # 상관없음은 부족한 역할을 메울 수 있지만 우선순위는 낮게
    if pos == "상관없음":
        missing = 0
...
class Team(commands.Cog):
    ...
    async def balance_team(self, interaction: discord.Interaction):
        ...
        if lobby["game"] == "overwatch" and lobby["team_size"] == 5:
            team_a, team_b = make_overwatch_balanced_teams(selected, lobby["team_size"])
            mode_text = "오버워치 역할 균형 + MMR 분배"
        else:
            team_a, team_b = split_by_mmr(selected, lobby["team_size"])
            mode_text = "기본 MMR 분배"
