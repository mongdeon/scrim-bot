...
class Settings(commands.Cog):
    ...
    async def role(self, interaction: discord.Interaction, 역할: discord.Role):
        db.set_role(interaction.guild_id, 역할.id)
    ...
    async def cat(self, interaction: discord.Interaction, 카테고리: discord.CategoryChannel):
        db.set_category(interaction.guild_id, 카테고리.id)
    ...
    async def show(self, interaction: discord.Interaction):
        ...
        role = interaction.guild.get_role(data["role_id"]) if data["role_id"] else None
