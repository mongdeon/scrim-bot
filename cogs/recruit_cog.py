...
class RecruitCreateModal(discord.ui.Modal, title="내전 생성 상세 설정"):
    def __init__(self, cog, game_val: str, game_name: str, final_team_size: int, final_total_slots: int | None, match_format: str, series_target: int, tournament_stage: str, pubg_mode_name: str | None):
        super().__init__()
        self.cog = cog
        self.game_val = game_val
        self.game_name = game_name
        self.final_team_size = final_team_size
        self.final_total_slots = final_total_slots
        self.match_format = match_format
        self.series_target = series_target
        self.tournament_stage = tournament_stage
        self.pubg_mode_name = pubg_mode_name

    time_input = discord.ui.TextInput(
        label="시작 시간 (생략 시 현재 시간)",
        style=discord.TextStyle.short,
        placeholder="예: 20:00 (생략 시 즉시 시작)",
        required=False,
        max_length=20
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        lobby = db.create_lobby(
            ...
            interaction.user.id,
            self.game_val,
            self.final_team_size,
            total_slots=self.final_total_slots,
            scheduled_at=scheduled_at,
            match_format=self.match_format,
            series_target=self.series_target,
        ...
        if self.game_val == "pubg":
            log_desc = (
                f"생성자: {interaction.user.mention}\n채널: {interaction.channel.mention}\n"
                f"로비 ID: {lobby['lobby_id']}\n"
                f"게임: PUBG\n형식: {self.pubg_mode_name if self.pubg_mode_name else '-'}\n총 모집 인원: {self.final_total_slots}"
            )
        else:
            ...
            log_desc = (
                f"생성자: {interaction.user.mention}\n채널: {interaction.channel.mention}\n"
                f"로비 ID: {lobby['lobby_id']}\n"
                f"게임: {self.game_name}\n팀 인원: {self.final_team_size} vs {self.final_team_size}\n진행: {extra_format}"
            )
...
class Recruit(commands.Cog):
    ...
    async def create(
        ...
        try:
            ...
            if 게임.value == "pubg":
                if 배그형식 is None:
                    await interaction.response.send_message("배그는 `배그형식`을 선택해야 합니다. (솔로 / 듀오 / 스쿼드)", ephemeral=True)
                    return

                if 모집인원 is None:
                    await interaction.response.send_message("배그는 `모집인원`을 입력해야 합니다.", ephemeral=True)
                    return
            ...
            modal = RecruitCreateModal(
                ...
                final_total_slots=final_total_slots,
                match_format=match_format,
                series_target=series_target,
                tournament_stage=tournament_stage,
                pubg_mode_name=배그형식.name if 배그형식 else None
            )
    ...
    @app_commands.command(name="내전시간수정", description="팝업창을 통해 직관적으로 내전 시작 시간을 수정합니다.")
    ...
    async def edit_lobby_time(
        ...
        try:
            ...
            modal = RecruitEditTimeModal(cog=self, lobby=lobby)
            await interaction.response.send_modal(modal)
